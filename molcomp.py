...
bond_distances={'H-H':0.74,'H-C':1.09,'H-N':1.01,'H-O':0.96, 'H-F':0.92, 'H-Cl':1.27,'H-Br':1.41,'H-I':1.61,	 
                'C-C':1.54,'C--C':1.34,'C---C':1.20,'C-N':1.47,'C-O':1.43,'C-S':1.82,'C-F':1.35,'C-Cl':1.77,'C-Br':1.94,'C-I':2.14,    
                'N-N':1.45,'N---N':1.10,                                       
                'O-O':1.48,'O--O':121,                                          
                'F-F':1.42,	
                'Cl-Cl':1.99,	
...
longjoin_space =lambda x,y:" ".join(["".join((item[0],mul(item[1],y))) for item in x])
...
class c_formula():
    ...
    def __init__(self, Formula):
        if isinstance(Formula , dict):
            ...
            self.brute= ' '.join([x[0]+str(x[1]) for x in list(Formula.items())])        
        else:           
    ...
    while True:
        ...
        for i in formula2:
            if i == "M": print("\nMolecular weight:\n ",sample.MW," uma") 
                ...
                print(sample.Omol, "Mole of O in ", gram, " grams of ", formula)
            elif i=="E":
                ...
                jiter=0
                while percent18<.99:
                    percent18=exchange(sample.Omol,percent18,float(v_mole),1)
                    jiter+=1
                    print("Exchange ",jiter, "O18 present in the sample ",round(percent18*100,2),"%")  
            pass
            print("\n\n\n\n")
